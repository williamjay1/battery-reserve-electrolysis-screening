...
config=dict(stage='main',coverage=[expected[0],expected[-1]],normalization='2025 H1 absolute 99.5 percentile; clipped signed setpoint',scale_mw=scale,eta=.94,horizon_hours=1.,capacity_mwh=[.25,.5,1.,2.,4.],reserve_mw=[.25,.5,.75],scheduled_supply_mw=[.25,.5,.75],inverter_mw=1.,electrolyzer_mw=1.,models=models,reliability_thresholds=[.001,.005,.01],primary_threshold=.005,uncertainty='paired calendar 7-day block bootstrap; 3/14-day sensitivity',interpretation='system signal stress replay, not participant activation or qualification')
(ROOT/'results'/'germany_main_config.json').write_text(json.dumps(config,indent=2))
...
for di,f in enumerate(files):
    ...
    for k,(cap,r,supply,label,step,mode) in enumerate(configs):
        ...
        out=replay_day(a,cap,r,.94,supply,1.,step,mode,state[k,0],state[k,1]);state[k]=out[:2]
...
df=pd.DataFrame(rows,columns=columns);df.to_csv(ROOT/'results'/'germany_main_daily.csv.gz',index=False)
...
(ROOT/'results'/'germany_main_audit.json').write_text(json.dumps(dict(days=len(files),configurations=len(configs),rows=len(df),elapsed_seconds=time.perf_counter()-t0,native_envelope_max_errors=errors,status='PASS'),indent=2))
